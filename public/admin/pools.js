...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  function setActiveSystem(sys) {
    ...
    if (sysPortalBtn) {
      sysPortalBtn.className = "filter-btn" + (activeSystem === "portal" ? " primary" : "");
    }
    if (sysMikrotikBtn) {
      sysMikrotikBtn.className = "filter-btn" + (activeSystem === "mikrotik" ? " primary" : "");
    }
  ...
  function updateCreateFieldsVisibility() {
    const sys = (newSystemEl?.value || activeSystem) === "mikrotik" ? "mikrotik" : "portal";
    const isM = sys === "mikrotik";

    if (newMikrotikIpEl) {
      newMikrotikIpEl.style.display = isM ? "" : "none";
      newMikrotikIpEl.required = isM;
    }
    if (newRadiusNasIdEl) {
      newRadiusNasIdEl.style.display = isM ? "" : "none";
    }
  }

  if (!(await guardSession(meEl))) return;

  sysPortalBtn?.addEventListener("click", () => {
    setActiveSystem("portal");
    loadPools().catch((err) => showMsg(msgEl, err.message, true));
  });

  sysMikrotikBtn?.addEventListener("click", () => {
  ...
  async function fetchPoolLiveStats() {
    try {
      ...
      for (const row of rows) {
        const poolIdKey = normId(row?.pool_id);
        const normalized = {
          pool_id: String(row?.pool_id ?? "").trim(),
          ...
          radius_nas_id: String(row?.radius_nas_id ?? "").trim() || null,
        };

        if (poolIdKey) byPoolId.set(poolIdKey, normalized);
        allRows.push(normalized);
      }

      return { byPoolId, allRows };
    } catch (e) {
      console.error("pool-live-stats error", e);
      return { byPoolId: new Map(), allRows: [] };
    }
  }

  function findStatsForPool(pool, liveStats) {
    const poolIdKey = normId(pool?.id);
    if (poolIdKey && liveStats.byPoolId.has(poolIdKey)) {
      return liveStats.byPoolId.get(poolIdKey);
    }

    const poolNasId = String(pool?.radius_nas_id ?? pool?.radiusNasId ?? "").trim();
    if (poolNasId) {
      const poolNasKey = normId(poolNasId);
      const viaNas = liveStats.allRows.find((row) => normId(row?.radius_nas_id) === poolNasKey);
      if (viaNas) return viaNas;
    }
  ...
  async function loadPools() {
    ...
    const [liveStats, data] = await Promise.all([
      fetchPoolLiveStats(),
      fetchJSON(`/api/admin/pools?${query.toString()}`),
    ]);
    ...
    rowsEl.innerHTML = pools.map((p) => {
      ...
      const name = p?.name || "";
      const stats = findStatsForPool(p, liveStats);
      ...
      const titleBits = [];
      if (stats?.last_computed_at) titleBits.push(`Last update: ${stats.last_computed_at}`);
      if (stats?.is_stale) titleBits.push("stale");
      const liveTitle = titleBits.length ? ` title="${esc(titleBits.join(" · "))}"` : "";
